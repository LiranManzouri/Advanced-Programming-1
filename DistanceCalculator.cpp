...
double DistanceCalculator::manhattan(const Flower &f1, const Flower &f2) {
    ...
    double distance = 0;
    distance += abs(f1.getCalyxLeavesLength() - f2.getCalyxLeavesLength());
    distance += abs(f1.getCalyxLeavesWidth() - f2.getCalyxLeavesWidth());
    distance += abs(f1.getPetalLength() - f2.getPetalLength());
    distance += abs(f1.getPetalWidth() - f2.getPetalWidth());
    return distance;
}
